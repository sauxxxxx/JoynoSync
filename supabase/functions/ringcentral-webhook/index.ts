...
import { reconcileProviderCall, recordProviderEvent } from "../_shared/ringcentral-reconcile.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    const validationToken = normalizeText(req.headers.get("validation-token"));
    const payload = await req.json().catch(() => ({}));
    const isValidationChallenge = !Object.keys(payload as Record<string, unknown>).length;
    if (isValidationChallenge) {
      if (!validationToken) {
        return jsonResponse(req, 401, { ok: false, error: "RingCentral validation token is missing." });
      }
      return jsonResponse(req, 200, { ok: true, validation: true }, { "Validation-Token": validationToken });
    }

    const expectedToken = normalizeText(Deno.env.get("RINGCENTRAL_WEBHOOK_VALIDATION_TOKEN"));
    if (!expectedToken) {
      return jsonResponse(req, 503, { ok: false, error: "RingCentral webhook validation is not configured." });
    }
    if (validationToken !== expectedToken) {
      return jsonResponse(req, 401, { ok: false, error: "Invalid RingCentral webhook token." });
    }
    ...
    for (const party of notification.parties) {
      ...
      const callLog = await reconcileProviderCall({
        ...
        queueName: normalizeText((party as Record<string, unknown>).queueName || queue.name),
        direction,
        fromNumber,
        toNumber,
      ...
      await recordProviderEvent(
        ...
        normalizeText(callLog?.id),
        notification.eventType,
        payload,
        normalizeText((payload as Record<string, unknown>).uuid)
      );
      processed += 1;
    }
