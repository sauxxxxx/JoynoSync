...
async function fetchJob(serviceClient: ReturnType<typeof createServiceClient>, jobId: string) {
  const { data, error } = await serviceClient
    ...
    .select("id,workspace_id,created_by_member_id,status,duplicate_mode,import_mode,reset_blank_status,restore_archived,distribution_mode,assignee_ids,rows,row_count,processed_count,created_count,updated_count,skipped_count,assigned_count,left_unassigned_count,last_error,started_at,heartbeat_at")
    .eq("id", jobId)
    .single();
  if (error) {
    throw error;
