...
function serializeJob(row: LeadImportJobRow | null) {
  ...
  return {
    ...
    importMode: normalizeText(row.import_mode) || "new",
    resetBlankStatus: Boolean(row.reset_blank_status),
    restoreArchived: Boolean(row.restore_archived),
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      if (rows.length > 10_000) {
        return jsonResponse(req, 400, { ok: false, error: "Import jobs are limited to 10,000 rows" });
      }

      const importMode = normalizeText(payload.importMode) === "update-exported" ? "update-exported" : "new";
      if (importMode === "update-exported" && rows.some((row) => !normalizeText(row.values.leadId))) {
        return jsonResponse(req, 400, { ok: false, error: "Every exported-lead update row requires a Lead ID" });
      }
      if (importMode === "update-exported" && rows.some((row) => !normalizeText(row.values.updatedAt))) {
        return jsonResponse(req, 400, { ok: false, error: "Every exported-lead update row requires an Updated At version" });
      }
      ...
      const insertPayload = {
        ...
        reset_blank_status: Boolean(payload.resetBlankStatus),
        restore_archived: Boolean(payload.restoreArchived),
      ...
      const { data: insertedJob, error: insertError } = await serviceClient
        ...
        .select("id,workspace_id,created_by_member_id,file_name,status,duplicate_mode,import_mode,reset_blank_status,restore_archived,distribution_mode,distribution_method,assignee_ids,row_count,processed_count,created_count,updated_count,skipped_count,assigned_count,left_unassigned_count,last_error,started_at,completed_at,heartbeat_at,created_at,updated_at")
        .single();
      if (insertError) {
        throw insertError;
      }
