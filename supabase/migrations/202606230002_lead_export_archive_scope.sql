...
create or replace function private.normalize_lead_export_archive_scope(value text)
returns text
language sql
immutable
as $$
...
revoke all on function public.get_lead_export_rows(text, text, date, date, text, text) from public;
grant execute on function public.get_lead_export_rows(text, text, date, date, text, text) to authenticated;
