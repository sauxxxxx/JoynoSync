...
create table if not exists public.lead_export_batches (
...
create table if not exists public.lead_export_items (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces(id) on delete cascade,
  batch_id uuid not null references public.lead_export_batches(id) on delete cascade,
  export_type text not null,
  export_key text not null,
  lead_id uuid,
  created_at timestamptz not null default now(),
  unique (workspace_id, export_type, export_key)
);

create index if not exists lead_export_batches_workspace_created_idx
on public.lead_export_batches (workspace_id, export_type, created_at desc);

alter table public.lead_export_batches enable row level security;
alter table public.lead_export_items enable row level security;

drop policy if exists lead_export_batches_no_direct_access on public.lead_export_batches;
create policy lead_export_batches_no_direct_access
on public.lead_export_batches for all using (false) with check (false);

drop policy if exists lead_export_items_no_direct_access on public.lead_export_items;
create policy lead_export_items_no_direct_access
on public.lead_export_items for all using (false) with check (false);
...
create or replace function private.normalize_lead_export_scope(value text)
returns text
language sql
immutable
as $$
  select case lower(trim(coalesce(value, '')))
    when 'date-range' then 'date-range'
    when 'all' then 'all'
    else 'new'
  end;
$$;
...
revoke all on table public.lead_export_batches from public;
revoke all on table public.lead_export_items from public;
