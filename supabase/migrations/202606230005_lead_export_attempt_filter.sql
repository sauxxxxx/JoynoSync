...
create or replace function private.normalize_lead_export_attempt_filter(value text)
returns integer
language sql
immutable
as $$
...
create or replace function public.get_lead_export_rows(
  ...
  p_attempt_filter text default 'all'
)
returns jsonb
language plpgsql
security definer
...
declare
  ...
  normalized_archive_scope text := private.normalize_lead_export_archive_scope(p_archive_scope);
  normalized_attempt_filter integer := private.normalize_lead_export_attempt_filter(p_attempt_filter);
    ...
    with candidate_rows as (
      ...
      where l.workspace_id = target_workspace_id
        and (
          ...
          or (normalized_archive_scope = 'archived' and l.archived_at is not null)
          or (normalized_archive_scope = 'active' and l.archived_at is null)
        )
        and (
        ...
        and (
          normalized_type <> 'leads'
          or normalized_status <> 'Contacted'
          or normalized_attempt_filter is null
          or case
            when trim(coalesce(l.meta ->> 'attemptCount', '')) ~ '^[0-9]+$'
              then trim(coalesce(l.meta ->> 'attemptCount', ''))::integer
            else 0
          end = normalized_attempt_filter
        )
        and (
...
revoke all on function public.get_lead_export_rows(text, text, date, date, text, text, text) from public;
grant execute on function public.get_lead_export_rows(text, text, date, date, text, text, text) to authenticated;
