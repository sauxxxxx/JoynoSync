...
const smokeRoutes = [
  ...
  },
  {
    name: "integrations",
    hash: "#/integrations",
    expectedPatterns: [/integrations/i, /workspace access/i, /sign in/i]
  },
  {
    name: "invite",
...
test.describe("SPA smoke shell", () => {
  ...
  test("opens the localhost-only QA workspace without publishing credentials", async ({ page }) => {
    await page.goto("/#/login", { waitUntil: "domcontentloaded" });

    const localQaButton = page.getByRole("button", { name: "Open local QA workspace" });
    await expect(localQaButton).toBeVisible();
    await expect(page.locator("#app")).not.toContainText(/local QA password/i);

    await localQaButton.click();
    await page.waitForURL(/#\/dashboard$/);
    await expect(page.locator("#app")).toContainText(/Dashboard/i);
  });
});
