  ...
  if (isSyncingAccess) {
    ...
    panelBody = `
      ${
        localQaAvailable
          ? `<div class="auth-login-status is-info">
              <div class="auth-login-status-icon"><i class="bi bi-laptop" aria-hidden="true"></i></div>
              <div>
                <p class="auth-login-status-title">Local QA workspace</p>
                <p>Open browser-only test data for this computer. This local session cannot write to Supabase.</p>
                <button class="btn btn-light" type="button" data-action="local-qa-sign-in">Open local QA workspace</button>
              </div>
            </div>`
          : ""
      }
