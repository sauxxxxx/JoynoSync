...
function mapCallLog(entry) {
  return {
    ...
    followUpAt: normalizeIso(entry?.followUpAt),
    followUpTaskId: normalizeText(entry?.followUpTaskId),
    outcomeSubmittedAt: normalizeIso(entry?.outcomeSubmittedAt),
...
function mapTelephonyIdentity(entry) {
  return {
    ...
    extensionStatus: normalizeText(entry?.extensionStatus),
    active: normalizeBoolean(entry?.active),
    updatedAt: normalizeIso(entry?.updatedAt)
  };
}
...
export function startSupabaseCall(payload = {}) {
  return invokeCallsFunction("ringcentral-start-call", {
    ...
    sourceTaskId: normalizeText(payload.sourceTaskId)
  });
}
...
export async function deleteSupabaseCallLog(callLogId) {
  const response = await invokeSupabaseFunction("delete-call-log", {
    method: "POST",
    body: { callLogId: normalizeText(callLogId) }
  });
  return parseFunctionResponse(response);
}

export function answerSupabaseCall(callLogId) {
  return invokeCallsFunction("ringcentral-answer-call", { callLogId: normalizeText(callLogId) });
}

export function declineSupabaseCall(callLogId) {
  return invokeCallsFunction("ringcentral-decline-call", { callLogId: normalizeText(callLogId) });
}

export async function saveSupabaseCallWrapup(callLogId, payload = {}) {
  const client = getClient();
  const { data, error } = await client.rpc("complete_call_wrapup", {
    p_payload: {
      callLogId: normalizeText(callLogId),
      outcome: normalizeText(payload.disposition),
      notes: normalizeText(payload.wrapupNotes),
      followUpAction: normalizeText(payload.followUpAction, "none"),
      followUpAt: normalizeIso(payload.followUpAt),
      followUpLocal: normalizeText(payload.followUpLocal)
    }
  });
  if (error) {
    throw error;
