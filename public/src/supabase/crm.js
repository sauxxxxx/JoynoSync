...
import { hasLeadCursorExtraRow, normalizeLeadCursorPageRows } from "../modules/lead-pagination.js";
...
function resolveLatestWeeklyMaintenanceCutoff(referenceValue = new Date().toISOString()) {
  const parsed = Date.parse(String(referenceValue || "").trim());
  const reference = Number.isFinite(parsed) ? new Date(parsed) : new Date();
  const offsetMs = LEAD_WEEKLY_REMOVAL_TIMEZONE_OFFSET_HOURS * 60 * 60 * 1000;
  const local = new Date(reference.getTime() + offsetMs);
  const weekday = local.getUTCDay();
  const beforeFridayCutoff =
    weekday === LEAD_WEEKLY_REMOVAL_WEEKDAY &&
    local.getUTCHours() < LEAD_WEEKLY_REMOVAL_CUTOFF_HOUR;
  let daysSinceFriday = (weekday - LEAD_WEEKLY_REMOVAL_WEEKDAY + 7) % 7;
  if (beforeFridayCutoff) {
    daysSinceFriday = 7;
  }
  return new Date(
    Date.UTC(
      ...
      local.getUTCMonth(),
      local.getUTCDate() - daysSinceFriday,
...
function mapLeadRow(row, context) {
  ...
  return {
    ...
    lastTouchAt: normalizeIso(row?.last_touch_at),
    lastCallOutcome: normalizeText(row?.last_call_outcome),
    crmConversationId: normalizeText(row?.crm_conversation_id),
    notes: normalizeText(row?.notes),
    tags: normalizeTagArray(row?.tags),
...
export async function fetchSupabasePendingFridayRedistributionLeads(workspaceId) {
  ...
  const rows = [];
  let from = 0;
  const dueBefore = resolveLatestWeeklyMaintenanceCutoff();
...
export async function fetchSupabaseLeadsPage(workspaceId, options = {}) {
  ...
  if (!includeMeta) {
    const { data, error } = await client.rpc("get_leads_cursor_page", {
      ...
      p_sort_key: sortKey || "lastTouch",
      p_sort_dir: sortDir === "asc" ? "asc" : "desc",
      p_cursor_sort_value: cursorUsable ? requestedCursor.sortValue : null,
      p_cursor_id: cursorUsable ? requestedCursor.id : null,
      p_cursor_direction: cursorUsable ? cursorDirection : "next",
      p_today: localIsoDate(0)
    });
    if (!error && data && typeof data === "object") {
      rpcPageData = data;
    } else if (error && error.code !== "PGRST202" && !String(error?.message || "").includes("Could not find the function")) {
      throw error;
    }
  }
  if (!rpcPageData) {
    rowsResult = await rowsQuery;
    if (rowsResult.error) {
  ...
  const fetchedRows = rpcPageData
    ? (Array.isArray(rpcPageData.rows) ? rpcPageData.rows : [])
    : (Array.isArray(rowsResult?.data) ? rowsResult.data : []);
  const cursorWasApplied = rpcPageData ? Boolean(rpcPageData.cursorApplied) : Boolean(cursorUsable);
  const effectiveCursorDirection = cursorWasApplied
    ? normalizeText(rpcPageData?.cursorDirection || cursorDirection, "next")
    : "next";
  const hasExtraPage = Boolean(rpcPageData?.hasMore) || hasLeadCursorExtraRow(fetchedRows, pageSize);
  const pageRows = normalizeLeadCursorPageRows(fetchedRows, pageSize, effectiveCursorDirection);
  let hasNextPage = false;
  let hasPreviousPage = false;
  if (cursorWasApplied && effectiveCursorDirection === "prev") {
    hasPreviousPage = hasExtraPage;
    hasNextPage = true;
  } else if (cursorWasApplied) {
...
export async function fetchSupabaseLeadsPageMeta(workspaceId, options = {}) {
  ...
  if (!rpcMetaError && rpcMeta && typeof rpcMeta === "object") {
    const waitingItems = (Array.isArray(rpcMeta.waitingItems) ? rpcMeta.waitingItems : []).map((row) => ({
      id: normalizeText(row?.id),
      title: [normalizeText(row?.name), normalizeText(row?.companyName)].filter(Boolean).join(" | ") || "Lead",
      owner: resolveOwnerName(row?.ownerMemberId, memberNameMap) || "Unassigned",
      linkedType: `Due ${normalizeDateOnly(row?.nextFollowUp)}`
    }));
    return {
      totalCount: Number(rpcMeta.totalCount || 0),
      scopeCounts: {
        all: Number(rpcMeta.scopeCounts?.all || 0),
        mine: Number(rpcMeta.scopeCounts?.mine || 0),
        unassigned: Number(rpcMeta.scopeCounts?.unassigned || 0),
        assigned: Number(rpcMeta.scopeCounts?.assigned || 0)
      },
      reserveCount: Number(rpcMeta.reserveCount || 0),
      sourceOptions: [
        { id: "all", label: "All" },
        ...(Array.isArray(rpcMeta.sources) ? rpcMeta.sources : [])
  ...
  if (rpcMetaError && !String(rpcMetaError?.message || "").includes("Could not find the function")) {
    console.warn("Supabase leads metadata RPC failed; falling back to parallel queries:", rpcMetaError);
  }
