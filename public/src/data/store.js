...
const LOCAL_QA_STORAGE_KEY = "joyno_local_qa_data_v1";

function getStorageKey() {
  return isLocalQaSessionActive() ? LOCAL_QA_STORAGE_KEY : STORAGE_KEY;
}

function cloneSeed() {
  const cloned = structuredClone(seedData);
  return isLocalQaSessionActive() ? cloned : stripEphemeralCollections(cloned);
}
...
function normalizeData(data) {
  ...
  return isLocalQaSessionActive() ? normalized : stripEphemeralCollections(normalized);
}
